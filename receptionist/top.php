...
$sql1 = "SELECT res_datetime,table_number,register FROM `tbl_res` WHERE reserve = TRUE AND DATE(res_datetime) = $date AND register > 0 ";
$sql2 = "SELECT res_number FROM `tbl_res` WHERE DATE(res_datetime) = '$date' AND hold = 0 AND register = 1 ORDER BY res_datetime ASC;";
$sql4 = "SELECT res_number FROM `tbl_res` WHERE DATE(res_datetime) = '$date' AND hold = 1";

///0<受付待ちの人数、1<予約日付、2<受付済みの利用客を早い順番に検索 
$rs = $con->query($sql);
